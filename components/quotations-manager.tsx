...
import { Plus, Search, Eye, FileText, ArrowRight, Printer, Share2 } from "lucide-react"
...
export default function QuotationsManager() {
  ...
  useEffect(() => {
    if (allBills) {
      const quotationsOnly = allBills.filter((bill) => bill.type === "quotation")
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Quotation for ${quotation.customerName} - Total: ₹${Number.parseFloat(quotation.total).toFixed(2)}`,
  ...
  const convertToBill = async (quotation: Bill) => {
    ...
    try {
      const billNumber = await billsService.generateBillNumber(companyId, "bill")
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Quotations Management</h1>
          ...
          <Card key={quotation.id}>
            <CardHeader>
              <div className="flex justify-between items-start">
                <div>
                  <CardTitle className="text-lg">Quotation #{quotation.billNumber}</CardTitle>
                  <CardDescription>
                    {quotation.customerName} • {new Date(quotation.createdAt).toLocaleDateString()}
                  </CardDescription>
                </div>
                <div className="flex items-center gap-2">
                  <Badge variant={getStatusColor(quotation.status)}>{quotation.status}</Badge>
                  <span className="text-lg font-bold">₹{Number.parseFloat(quotation.total).toFixed(2)}</span>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              <div className="flex justify-between items-center">
                <div className="text-sm text-muted-foreground">
                  {Array.isArray(quotation.items) ? quotation.items.length : 0} item(s) • Subtotal: ₹
                  {Number.parseFloat(quotation.subtotal).toFixed(2)} • Tax: ₹
                  {Number.parseFloat(quotation.taxAmount).toFixed(2)}
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => handleViewQuotation(quotation)}>
                    <Eye className="h-3 w-3 mr-1" />
                    View
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => handleShare(quotation)}>
      ...
      <Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex justify-between items-center">
              <div>
                <DialogTitle>Quotation #{selectedQuotation?.billNumber}</DialogTitle>
                <DialogDescription>Quotation details for {selectedQuotation?.customerName}</DialogDescription>
              </div>
              <div className="flex gap-2">
                ...
                <Button variant="outline" size="sm" onClick={() => selectedQuotation && handleShare(selectedQuotation)}>
          ...
          {selectedQuotation && company && <PrintTemplate ref={printRef} bill={selectedQuotation} company={company} />}
        </DialogContent>
      </Dialog>
    </div>
  )
